...
from apps.container_order.models import CounterPartyOrder, ContainerOrder
...
class CounterPartyOrderCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        if CounterPartyOrder.objects.filter(
            order__order_number=data["order"]["order_number"],
            category_id=data["category_id"],
            counterparty_id=data["counterparty_id"],
        ).exists():
            raise serializers.ValidationError(
                "Order with this category and counterparty already exists"
            )
        return data
    ...
    def create(self, validated_data):
        ...
        counterparty = CounterPartyOrder.objects.create(
            **validated_data, order=container_order.order
        )
        return counterparty
