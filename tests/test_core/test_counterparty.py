...
def test_detail_counterparty(client, counterparty):
    response = client.get(f"/counterparty/counterparties/{counterparty.id}/")
...
def test_list_counterparty(client, counterparty):
    response = client.get("/counterparty/counterparties/")
    response.json()['name'] = counterparty.name
    assert response.status_code == 200
...
def test_create_counterparty(client):
    response = client.post('/counterparty/counterparties/',
...
def test_update_counterparty(client, counterparty):
    response = client.put(f'/counterparty/counterparties/{counterparty.id}/',
...
def test_delete_counterparty(client, counterparty):
    response = client.delete(f'/counterparty/counterparties/{counterparty.id}/')
