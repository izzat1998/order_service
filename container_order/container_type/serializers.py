...
class ContainerTypeOrderCreateSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        container_order = ContainerOrder.objects.filter(
            order__order_number=validated_data.pop('order').pop('order').pop('order_number')).first()
        return ContainerTypeOrder.objects.create(**validated_data, order=container_order)
